"""
📊 日次カロリー・PFCダッシュボード
Streamlit マルチページアプリ用ページ

使い方:
  pfc-app/pages/dashboard.py として配置してください。
  Streamlit が自動的にサイドバーにページとして表示します。
"""
...
# --- ページ設定 ---
st.set_page_config(page_title="📊 PFCダッシュボード", layout="centered")
...
def _load_bg_image():
    bg_path = _pathlib.Path(__file__).resolve().parent.parent / "bg.png"
    if bg_path.exists():
...
def create_pfc_chart(df, chart_type):
    ...
    for key in ["protein", "fat", "carb"]:
        if chart_type == "棒グラフ":
            fig.add_trace(go.Bar(
                x=df["label"], y=df[key],
                marker_color=colors[key], name=names[key],
                marker_line_width=0,
            ))
        elif chart_type == "エリア":
            fig.add_trace(go.Scatter(
...
def create_pfc_pie(today_row):
    p_cal = today_row["p_cal"]
    f_cal = today_row["f_cal"]
    c_cal = today_row["c_cal"]
...
def main():
    ...
    with col_range:
        days = st.radio("表示期間", [7, 14, 30], index=1, horizontal=True, format_func=lambda d: f"{d}日間")
    with col_chart:
        chart_type = st.radio("グラフ形式", ["棒グラフ", "エリア", "折れ線"], horizontal=True)
    ...
    cal_fig = create_calorie_chart(df, target_cal, chart_type)
    st.plotly_chart(cal_fig, use_container_width=True, config={"staticPlot": True})

    # --- PFC推移 ---
    st.subheader("🏋️ PFCバランス推移 (g)")
    pfc_fig = create_pfc_chart(df, chart_type)
    st.plotly_chart(pfc_fig, use_container_width=True, config={"staticPlot": True})
    ...
    with col_stack:
        ...
        stack_fig = create_stacked_cal_chart(df)
        st.plotly_chart(stack_fig, use_container_width=True, config={"staticPlot": True})
